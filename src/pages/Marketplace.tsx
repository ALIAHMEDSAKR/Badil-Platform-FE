...
function getMaterialTag(materialType: string): {
  ...
  if (
    ...
    return {
      label: "Polymer",
      color: "bg-[#2dd4bf]/15 text-[#2dd4bf] border-[#2dd4bf]/25",
    };
  if (
  ...
  return {
    label: materialType,
    color: "bg-gray-700/30 text-gray-400 border-gray-600/25",
  };
}
    ...
    >
      ...
      <div className="p-4">
        ...
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="text-[11px] text-gray-500">• Listing</span>
        </div>
  ...
  const filtered = useMemo(() => {
    ...
    if (activeCategory !== "All Materials") {
      ...
      result = result.filter((l) => {
        ...
        return (
          tag.includes(catLower.slice(0, -1)) ||
          l.materialType.toLowerCase().includes(catLower.slice(0, -1))
        );
      });
    }
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
          {Array.from({ length: 8 }).map((_, i) => (
            <div
              key={i}
              className="bg-[#0f2424] border border-[#1e3a3a] rounded-xl overflow-hidden animate-pulse"
            >
